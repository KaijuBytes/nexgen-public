...
from nex.human_resources.doctype.holiday_list.holiday_list import local_country_name
...
@contextmanager
def set_holiday_list(holiday_list, agency_name):
	"""
	Context manager for setting holiday list in tests
	"""
	try:
		agency = frappe.get_doc("Agency", agency_name)
		previous_holiday_list = agency.default_holiday_list

		agency.default_holiday_list = holiday_list
		agency.save()

		yield

	finally:
		# restore holiday list Human Resources
		agency = frappe.get_doc("Agency", agency_name)
		agency.default_holiday_list = previous_holiday_list
		agency.save()
